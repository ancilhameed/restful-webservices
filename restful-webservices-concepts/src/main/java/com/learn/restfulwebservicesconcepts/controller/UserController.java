...
public class UserController {
	...
	public EntityModel<User> retrieveUser(@PathVariable long id) {
		...
		// If we want to add useful links to our GET /users API then we need to use
		// HATEOAS
		// We'll make use of EntityModel to return our Resource with useful links
		EntityModel<User> userEntityModel = EntityModel.of(user);

		// If we just return EntityModel we wont get links, for adding links we need
		// WebMvcLinkBuilder
		...
		// Instead of Hard Coding the URL we creates a link using method name, now we
		// need to set it to EntityModel
